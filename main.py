        ...
        for a in range(len(Table[Day])-1):
            ...
            if a>=0:
                ...
                if time(int(end_break[0]),int(end_break[1]))>time(int(next_st_break[0]),int(next_st_break[1])):
                    ...
                    Options=("You may choose some other time slot for your task from the following options",FreeTimeSlots)
                    print(Statement)
        ...
        for j in range(len(Reminders[i])):
            if j==1:                                                               #stopping at the second column where user has entered the days
                ...
                for day in Repeat:                                                 #Repeat is a list containing full form of days entered by user(repition days)
                    ...
                    Todolist[day].append([Reminders[i][0],Reminders[i][2]])          #appending event,start time,end time
...
label_frame = tk.Label(root, text=Time,font=("Arial",35),fg="brown4").place(x=222,y=10)
label_frame = tk.Label(root, text=Date,font=("Arial",12),fg="brown4").place(x=235,y=65)
label_frame = tk.Label(root, text=Day,font=("Arial",10),fg="brown4").place(x=255,y=85)
...
label_frame = tk.Label(root, text="Task No.",font=("Arial",10),fg="brown4").place(x=75,y=355)
...
label_frame = tk.Label(root, text="Option No.",font=("Arial",10),fg="brown4").place(x=72,y=430)
...
box=Listbox(root,width=45,height=7)
box.place(x=125,y=380)
...
# def job(Todolist,Day):
#     for i in range(len(Todolist[Day])):
#             time=Todolist[Day][i][1]
#             Day=Day.lower()
#             schedule.every().Day.at(time).do(bell(Todolist,Day,i))
#
# def bell(Todolist,Day,i):
#     print(Todolist[Day][i])
#
# while True:
